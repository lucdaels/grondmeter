...
Gebruik: uv run --link-mode=copy deploy.py
"""
...
def deploy():
    print("[1/5] Token ophalen via service account...")
    token   = get_token()
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    base    = "https://firebasehosting.googleapis.com/v1beta1"

    print("[2/5] Nieuwe versie aanmaken...")
    ...
    print("[3/5] Bestanden indexeren...")
    file_hashes, file_data = {}, {}
    for fname in DEPLOY_FILES:
        fpath = os.path.join(SCRIPT_DIR, fname)
        if not os.path.exists(fpath):
            print(f"      WAARSCHUWING: {fname} niet gevonden, overgeslagen")
            continue
    ...
    print(f"      {len(to_upload)} bestand(en) te uploaden")
    ...
    print("[5/5] Versie afronden en release aanmaken...")
    requests.patch(f"{base}/{version_name}?update_mask=status",
        headers=headers, json={"status": "FINALIZED"}).raise_for_status()
    requests.post(f"{base}/sites/{SITE_ID}/releases?versionName={version_name}",
